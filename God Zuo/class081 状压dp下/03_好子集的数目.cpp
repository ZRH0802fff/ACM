...
public:
    ...
    int numberOfGoodSubsets1(vector<int>& nums) {
        ...
        vector<vector<int>> mem(MAXV + 1, vector<int>(LIMIT, -1));
        int ans = 0;
        for (int s = 1; s < LIMIT; s++) {
            ans = (ans + f1(MAXV, s, cnt, mem)) % MOD;
        }
        return ans;
    }
    ...
    // 质数: 29 23 19 17 13 11 7 5 3 2
    // 位置: 9 8 7 6 5 4 3 2 1 0
    int f1(int i, int s, vector<int>& cnt, vector<vector<int>>& mem) {
        if (mem[i][s] != -1) {
            return mem[i][s];
        }
        int ans = 0;
        if (i == 1) {
            ...
            ans = f1(i - 1, s, cnt, mem);
            int cur = own[i];
            int times = cnt[i];
            if (cur != 0 && times != 0 && (s & cur) == cur) {
                // 能要i这个数字
                ans = (int)(((long long)f1(i - 1, s ^ cur, cnt, mem) * times + ans) % MOD);
            }
        }
        mem[i][s] = ans;
        return ans;
